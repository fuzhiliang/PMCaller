...
###perl /56T/Evan/PMCaller/script/PMCaller_v4.pl -vcf /56T/Evan/PMCaller/mydata/chr22/outv4/chr22.P10T_1.sort.vcf -o /56T/Evan/PMCaller/mydata/chr22/outv4.1 -L 3:9:15:21 -cut_H 40 -window 10 -bam /56T/Evan/PMCaller/mydata/chr22/chr22.P10T_1.sort.bam &
###############################################################################
my ($sam,$bam,$L_list,$cut_off,$vcf,$out,$genome,$threads,$cut_H,$windows,$downsample,$quality,$reads_cut);
my %opts;
GetOptions(
    'verbose' => \$verbose,
    'sam=s' => \$sam,
...
if (-f "$out/errortable/$sample.gt") {
	...
	$cmd .="perl $Bin/bin/gy_sam_Error_Table_High_Low_v2.1.pl $out/$sample.snp.gy   $bam  $out/errortable/  $L_list 50";
	if ($quality) {
...
foreach my $L (@L_list) {
	$cmd .="awk '{if (\$1==50 && \$2==$L){print \$3\"\\t\"\$4\"\\t\"\$5\"\\t\"\$6\"\\t\"\$5/\$6}}' $out/errortable/$sample.errortable.list > $out/errortable/$sample.errortable.list_$L \n";
	$cmd .="awk '{if (\$1==50 && \$2==$L){print \$3\"\\t\"\$4\"\\t\"\$5\"\\t\"\$6\"\\t\"\$5/\$6}}' $out/errortable/$sample.HighQual.errortable.list > $out/errortable/$sample.errortable.list_$L.HighQual \n";
	$cmd .="awk '{if (\$1==50 && \$2==$L){print \$3\"\\t\"\$4\"\\t\"\$5\"\\t\"\$6\"\\t\"\$5/\$6}}' $out/errortable/$sample.LowQual.errortable.list > $out/errortable/$sample.errortable.list_$L.LowQual  \n";
	$cmd .="awk '{if (\$1==50 && \$2==$L){print \$3\"\\t\"\$4\"\\t\"\$5\"\\t\"\$6\"\\t\"\$5/\$6}}' $out/errortable/$sample.EXON.errortable.list > $out/errortable/$sample.errortable.list_$L.EXON \n";
	$cmd .="awk '{if (\$1==50 && \$2==$L){print \$3\"\\t\"\$4\"\\t\"\$5\"\\t\"\$6\"\\t\"\$5/\$6}}' $out/errortable/$sample.INT.errortable.list > $out/errortable/$sample.errortable.list_$L.INT  \n";
}
...
if (defined $quality){
	...
	#$cmd .="awk 'NR==FNR{a[\$1\"\\t\"\$2\"\\t\"\$3]=\$0}NR>FNR{if(\$1\"\\t\"\$2\"\\t\"\$3 in a){print a[\$1\"\\t\"\$2\"\\t\"\$3]\"\\t\"\$0}}' $out/down$a/$sample.sorted.DP$a.HighQual.qual$quality.vcf.modify $out/down$a/$sample.sorted.DP$a.LowQual.qual$quality.vcf.modify|cut -f 1-7,11-|awk '{if(NR==1){printf \$0\"\\tTDepth\\tDRef_HighQual\\tDAlt_HighQual\\tDRef_LowQual\\tDAlt_LowQual\\n\"}else{printf \$0\"\\t\"\$5+\$9\"\\t\"\$6\"\\t\"\$7\"\\t\"\$10\"\\t\"\$11\"\\n\"}}'|cut -f 1-4,12- > $out/down$a/$sample.sorted.DP$a.HighQual.LowQual.qual20.share.merge \n";
	$cmd .="perl /56T/Evan/PMCaller/script/PMCaller_genotype/merge.modify.pl   $out/down$a/$sample.sorted.DP$a.HighQual.qual$quality.vcf.modify $out/down$a/$sample.sorted.DP$a.LowQual.qual$quality.vcf.modify  $out/down$a/$sample.sorted.DP$a.HighQual.LowQual.qual20.share.merge \n";
	$cmd .="awk 'NR==FNR{a[\$1\"\\t\"\$2\"\\t\"\$3]=\$0}NR>FNR{if(\$1\"\\t\"\$2\"\\t\"\$3 in a){}else{if (\$4 !=\"NN\"){print \$0\"\\t0\\t0\\t\"\$6\"\\t\"\$7}}}' $out/down$a/$sample.sorted.DP$a.HighQual.qual$quality.vcf $out/down$a/$sample.sorted.DP$a.LowQual.qual$quality.vcf.modify  |cut -f 1-5,8- > $out/down$a/$sample.sorted.DP$a.HighQual.LowQual.qual20.LowQual.unique  \n ";
	$cmd .="awk 'NR==FNR{a[\$1\"\\t\"\$2\"\\t\"\$3]=\$0}NR>FNR{if(\$1\"\\t\"\$2\"\\t\"\$3 in a){}else{if (\$4 !=\"NN\"){print \$0\"\\t0\\t0\"}}}' $out/down$a/$sample.sorted.DP$a.LowQual.qual$quality.vcf.modify $out/down$a/$sample.sorted.DP$a.HighQual.qual$quality.vcf.modify > $out/down$a/$sample.sorted.DP$a.HighQual.LowQual.qual20.HighQual.unique  \n" ;
	$cmd .="cat $out/down$a/$sample.sorted.DP$a.HighQual.LowQual.qual20.share.merge  $out/down$a/$sample.sorted.DP$a.HighQual.LowQual.qual20.LowQual.unique $out/down$a/$sample.sorted.DP$a.HighQual.LowQual.qual20.HighQual.unique |awk '{if(NR==1){print \$0}else{if(\$6+\$8>=1 && \$7+\$9>=1){print \$0}}}' > $out/down$a/$sample.sorted.DP$a.HighQual.LowQual.qual20.dubious.merge \n";
	&runcmd("quality",$cmd);
	foreach my $L (@L_list) {
		$cmd ="";
		$cmd .=" perl $Bin/PMCaller_genotype/PMCaller_genotype_High_Low.pl -vcfmerge  $out/down$a/$sample.sorted.DP$a.HighQual.LowQual.qual20.dubious.merge  -o $out/PMCallerdown$L -snptable  $out/snptable/H${cut_H}_L$L/$sample.exp.snp.table_list  -errortablepre $out/errortable/$sample.errortable.list_$L  -q $quality \n";
		$cmd .=" awk 'NR==FNR{a[\$1\"\\t\"\$2\"\\t\"\$3]=\$4}NR>FNR{if (\$1\"\\t\"\$2\"\\t\"\$3 in a){print \$0\"\\t\"a[\$1\"\\t\"\$2\"\\t\"\$3]}else{print \$0\"\\tNN\"}}'  $out/PMCallerdown$L/genotype.HighQual_LowQual.qual20   $out/PMCallerdown$L/dubious_v4  > $out/PMCallerdown$L/HighQual_LowQual.qual20_pmcalller_vs_GT \n ";
		$cmd .=" awk 'NR==FNR{a[\$1\"\\t\"\$2\"\\t\"\$4]=\$8}NR>FNR{if (\$1\"\\t\"\$2\"\\t\"\$3 in a){print \$0\"\\t\"a[\$1\"\\t\"\$2\"\\t\"\$3]}else{print \$0\"\\t\"\$3\$3}}'  $out/down$a/$sample.DP$a.gatk.sed   $out/PMCallerdown$L/HighQual_LowQual.qual20_pmcalller_vs_GT  >  $out/PMCallerdown$L/gatk_vs_HighQual_LowQual.qual20_pmcalller_vs_GT  \n";
		$cmd .="mkdir $out/result \n" unless (-d "$out/result ");
		$cmd .="awk '{if (\$6==\$7){}else{print \$0}}' $out/PMCallerdown$L/gatk_vs_HighQual_LowQual.qual20_pmcalller_vs_GT |wc -l > $out/result/$L.HighLowErrorcount \n " ;
		$cmd .="awk '{if (\$6==\$8){}else{print \$0}}' $out/PMCallerdown$L/gatk_vs_HighQual_LowQual.qual20_pmcalller_vs_GT |wc -l >> $out/result/$L.HighLowErrorcount \n ";
		$cmd .="cat $out/PMCallerdown$L/gatk_vs_HighQual_LowQual.qual20_pmcalller_vs_GT |wc -l >> $out/result/$L.Errorcount \n " ;
...
foreach my $L (@L_list) {
	...
	$cmd .="mkdir $out/result \n" unless (-d "$out/result ");
	$cmd .="awk '{if (\$6==\$7){}else{print \$0}}' $out/PMCallerdown$L/gatk_pmcaller_vs_GT |wc -l >$out/result/$L.Errorcount \n " ;
	$cmd .="awk '{if (\$6==\$8){}else{print \$0}}' $out/PMCallerdown$L/gatk_pmcaller_vs_GT |wc -l >> $out/result/$L.Errorcount \n " ;
	$cmd .="cat $out/PMCallerdown$L/gatk_pmcaller_vs_GT |wc -l >> $out/result/$L.Errorcount \n " ;
...
foreach my $L (@L_list) {
	...
	$cmd .="mkdir $out/result \n" unless (-d "$out/result ");
	$cmd .="awk '{if (\$6==\$7){}else{print \$0}}' $out/PMCallerdown$L/EXONINT.gatk_pmcaller_vs_GT |wc -l >$out/result/$L.Errorcount \n " ;
	$cmd .="awk '{if (\$6==\$8){}else{print \$0}}' $out/PMCallerdown$L/EXONINT.gatk_pmcaller_vs_GT |wc -l >> $out/result/$L.Errorcount \n " ;
	$cmd .="cat $out/PMCallerdown$L/EXONINT.gatk_pmcaller_vs_GT |wc -l >> $out/result/$L.Errorcount \n " ;
...
Function: Template for Perl  通过bam或 bam和vcf文件call genotype，并跟GATK做比较
Command:      
	verbose	str	verbose,
	#sam		str sam,  
	-bam	str	bam,			 #必须的
...
sub runcmd2{
	...
	print S "$cmd ";
	print "Start analysis of $name... \n";
	system "nohup sh $out/shell/$name.sh &";
