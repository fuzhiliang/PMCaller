...
while (<V>) {
	...
	for (my $i=0;$i<=$#gt;$i++) {
		$ERROR{'H'}{$ref_UP}{$gt[$i]}=0 if (!exists $ERROR{'H'}{$ref_UP}{$gt[$i]});
		$ERROR{'L'}{$ref_UP}{$gt[$i]}=0 if (!exists $ERROR{'L'}{$ref_UP}{$gt[$i]});
		...
		#print A "$p=\t$SNP{$ref_UP}{$gt[$i]}*(($ERROR{'H'}{$ref_UP}{$gt[$i]}**HQual_Dref)*($ERROR{'H'}{$alt_UP}{$gt[$i]}**$HQual_DAlt)*($ERROR{'L'}{$ref_UP}{$gt[$i]}**LQual_Dref)*($ERROR{'L'}{$alt_UP}{$gt[$i]}**$LQual_DAlt))\n";
		if ($p >= $p_max) {
			$genotype=$gt[$i];
			$p_max=$p;
		}
