...
GetOptions(
	...
	'threads=i' => \$threads,
	'quality=i' => \$quality,
	'reads_cut=i'=>\$reads_cut,  #将reads 的前后$reads_cut  bp视为lowquality
) or die $!;
...
#	$cmd .="perl $Bin/bin/gy_sam_Error_Table_High_Low_v2.pl $out/$sample.snp.gy   $bam  $out/errortable/  $L_list 50 ";
...
if (defined $quality){
	...
	foreach my $L (@L_list) {
		...
		$cmd .="awk '{if (\$6==\$7){}else{print \$0}}' $out/PMCallerdown$L/gatk_vs_HighQual_LowQual.qual${quality}_pmcaller_vs_GT |wc -l > $out/result/$L.HighLowErrorcount \n " ;
		$cmd .="awk '{if (\$6==\$8){}else{print \$0}}' $out/PMCallerdown$L/gatk_vs_HighQual_LowQual.qual${quality}_pmcaller_vs_GT |wc -l >> $out/result/$L.HighLowErrorcount \n ";
		$cmd .="cat $out/PMCallerdown$L/gatk_vs_HighQual_LowQual.qual${quality}_pmcaller_vs_GT |wc -l >> $out/result/$L.Errorcount \n " ;
...
Command:      
	...
	-reads_cut	int	reads_cut,  #将reads 的前后$reads_cut  bp视为lowquality  [0]
