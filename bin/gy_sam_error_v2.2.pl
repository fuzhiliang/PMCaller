...
# /56T/Evan/PMCaller/mydata/chr22 & perl  /56T/Evan/PMCaller/script/bin/gy_sam_error_v2.1.pl  chr22.P10T_1.snp.gy  chr22.P10T_1.sorted.bam splittmp 3:10:21 50
my (@l)=(split ":" ,$l);
my (@h)=(split ":" ,$h);
my $maxski =&max(@l);
my $minski=&min(@l);
...
foreach my $H (keys %error) {
	foreach my $loc_t (keys %{$error{$H}}) {
		foreach my $gt (keys %{$error{$H}{$loc_t}}){
			print O "$loc_t\t$gt\t$H\t$error{$H}{$loc_t}{$gt}\n";
		}
	}
}
...
$cmd .= "perl $Bin/errortable_v1.pl $out/$sample.gt $out $l $sample  \n";
