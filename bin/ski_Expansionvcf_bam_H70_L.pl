...
###############################################################################
my ($L,$cut_peak2,$cut_peak3,$vcf,$out,$cut_H,$threads);
my %opts;
GetOptions(
    'verbose' => \$verbose,
...
unless(defined $vcf  && $L){&usage();exit 0;}
...
while (<I>) {
	chomp;
	next if (/^chrom/);
	my ($chr,$loc,$ref,$alt,$depth,$ref_depth,$alt_depth)= (split "\t", $_)[0,1,2,3,7,8,9] ; #  if ($cut_H <50 );
	#my ($chr,$loc,$ref,$alt,$depth,$ref_depth,$alt_depth)= (split "\t", $_)[0,1,2,3,4,5,6] if ($cut_H >=50 );  #扩增前真实数据削薄
	...
	next if ($cut_H >$depth);  #取固定H做error table 
...
sub usage {
    die(
        qq!
Usage:    perl $0  
Function: Template for Perl 基于扩增得到的*exp.vcf 文件，根据三个峰之间的两个cutoff，削薄的指定的L，得到削薄后的snp table 和error table
Command:      
    verbose      verbose
    'vcf=s' => \$vcf,   #假的vcf，是扩增后vcf_expansion_H60_120.pl 的中间文件
    'o=s' => \$out, 
    ...
    'cut_H=s' => \$cut_H,         #该深度以上的数据用来确定genotype 以及削薄
    'L=s' => \$L,				  #削薄到多少x
	'threads=i' => \$threads,     #假的，没用
